...
class Player(pygame.sprite.Sprite):
    ...
    def playerStats(self,screen):
        stats = self.font.render(f"Position: ({self.rect.x} ,{self.rect.y})    Velocity: ({round(math.hypot(self.vel_x,self.vel_y), 2)})    Accel: ({self.accel_x},{self.accel_y})   Angle: {math.degrees(self.angle)}",True,(0,255,0))
